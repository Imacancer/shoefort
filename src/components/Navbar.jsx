...
import { HeartIcon, MagnifyingGlassIcon, ShoppingBagIcon, ShoppingCartIcon, UserIcon, ArrowLeftOnRectangleIcon } from '@heroicons/react/24/outline'
        ...
        <nav className='flex items-center justify-between nike-container'>
            <div className='flex items-center'>
                <img
                    src={logo}
                    alt="logo/img"
                    className={`w-16 h-auto ${navState ? "glow-orange" : ""}`}
                />
            </div>
            <ul className='flex items-center justify-center gap-2'>
                <li className='grid items-center'>
                    <UserIcon className={`icon-style ${navState && "text-orange-500 transition-all duration-300"}`} />
                </li>
                <li className='grid items-center'>
                    <button type='button' onClick={onCartToggle} className='border-none outline-none active:scale-110 transition-all duration-300 relative'>
                        <ShoppingCartIcon className={`icon-style ${navState && "text-orange-500 transition-all duration-300"}`} />
